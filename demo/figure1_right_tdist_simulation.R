...
## Simulation code for manuscript Figure 1 (right) t-distribution without correction
#################################################################################################
...
sample_gen <- function(n, p, mu, omega, labels, hlambda, hsigma) {
  ...
  for (i in 1:n) {
    for (j in 1:p) {
      x[i, j] <- hlambda[j] * (alpha[i, labels[j]]) + hsigma[j] * rt(1, df)
    }
  }
