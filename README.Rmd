...
hbcm package contains all the necessary functions to conduct cluster analysis on network data by using the proposed heterogeneous block covariance model (HBCM).

## Installation

To the date (09/12/2021) the package is still under development. You can install the development version from [GitHub](https://github.com/) with:
...
1. Create a matrix data `x` of dimension `500x500`, with columns belonging to three non-overlapping groups (groups labeled as 1, 2, and 3). `x` values are determined by three things: parameter vector `alpha` (follows multivariate normal distribution), heterogeneous parameters vector `hlambda` and `hsigma`.
